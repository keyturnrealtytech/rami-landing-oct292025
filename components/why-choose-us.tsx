        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-8">
          {features.map((feature, index) => (
            <div key={index} className="text-center space-y-4">
              <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-[#81D8D0]/15 border border-[#81D8D0]/40 text-[#1f6b63] mb-2">
                <feature.icon className="h-7 w-7" />
              </div>
              <h3 className="text-xl font-semibold text-[#15211f]">{feature.title}</h3>
              <p className="text-sm text-[#5d6f6c] leading-relaxed">{feature.description}</p>
            </div>
          ))}
        </div>
      </div>
